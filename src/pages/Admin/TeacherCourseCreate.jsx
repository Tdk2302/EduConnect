    ...
    <Card title="Tạo TKB Giáo viên" style={{ maxWidth: 500, margin: "32px auto" }}>
      <Form form={form} layout="vertical" onFinish={onFinish}>
        ...
        <Form.Item name="slot" label="Khung giờ" rules={[{ required: true }]}>
          <Select options={SLOTS} placeholder="Chọn khung giờ" onChange={handleSlotOrDateChange} />
        </Form.Item>
        <Form.Item name="date" label="Ngày học" rules={[{ required: true }]}>
          <DatePicker style={{ width: "100%" }} format="YYYY-MM-DD" onChange={handleSlotOrDateChange} />
        </Form.Item>
        {startTime && endTime && (
          <div style={{ marginBottom: 16 }}>
            <b>Thời gian bắt đầu:</b> {startTime}<br />
            <b>Thời gian kết thúc:</b> {endTime}
          </div>
        )}
        <Form.Item>
          <Button type="primary" htmlType="submit" loading={loading} block disabled={!startTime || !endTime}>
            Tạo thời khóa biểu
          </Button>
        </Form.Item>
      </Form>
